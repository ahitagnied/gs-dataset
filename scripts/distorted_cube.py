...
def create_cube(config):
    ...
    # ─── subdivide to allow for organic bumpiness ───────────────────────────────
    subdiv = cube.modifiers.new(name="Subdiv", type='SUBSURF')
    subdiv.levels = 4  # viewport level of detail
    subdiv.render_levels = 4  # render-time detail
    bpy.ops.object.modifier_apply(modifier=subdiv.name)

    # ─── create a random bumpy surface using displacement ───────────────────────
    tex = bpy.data.textures.new("BumpTexture", type='MUSGRAVE')
    tex.musgrave_type = 'RIDGED_MULTIFRACTAL'
    tex.noise_scale = 0.6

    # add a displace modifier
    disp_mod = cube.modifiers.new(name="Displace", type='DISPLACE')
    disp_mod.texture = tex
    disp_mod.strength = config["cube"].get("bump_strength", 0.0015)  # overall height
    disp_mod.mid_level = 0.5  # default mid-level
